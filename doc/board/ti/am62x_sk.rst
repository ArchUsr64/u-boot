...
Falcon boot flow on am62x platforms bypasses the A53 SPL and u-boot with the
...
.. am62x_evm_falcon_start_build_process_tfa
...
.. am62x_evm_falcon_end_build_process_tfa
...
.. prompt:: bash $
 ...
 # use the falcon defconfig for am62x platforms
 export UBOOT_CFG_CORTEXR=am62x_evm_r5_falcon_defconfig
...
.. am62x_evm_falcon_start_build_process_fitimage
...
.. am62x_evm_falcon_end_build_process_fitimage
...
``tifalcon.bin`` with the Cortex-A SPL and it's corresponding DTBs removed.
...
.. am62x_evm_falcon_start_mmc_usage
...
``tifalcon.bin``, the DTB and fitImage (or kernel image for unauthenticated
...
.. am62x_evm_falcon_end_mmc_usage
...
Below commands can be used to download ``tiboot3.bin`` and ``tifalcon.bin``
over tftp and then flash those to OSPI at their respective addresses.
